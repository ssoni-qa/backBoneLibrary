	...
	public void valid_postal_code() throws InterruptedException, IOException
	{
		for (String stagingURL:OpenStreetMapLocatorPage.stagingURL) {
			...
			try {
				...
				testqa7.log(LogStatus.INFO, "Staging URL: "+stagingURL);
				...
				System.out.println(
						"Verification : 2. You entered an address which could not be geocoded [ 00000 ]. Please try another input address to search for locations.");
				//Getting aut url.
				...
				if (onPage.modalmsg.getText()
						.contains("You entered an address which could not be geocoded [ 0123456789")) {
				...
				e.printStackTrace();
				testqa7.log(LogStatus.FAIL, "Exception Occured. : " + e.getMessage() + " "
						+ testqa7.addScreenCapture(captureScreenMethod(dest)));
			} 
		}
	}
	@AfterMethod
