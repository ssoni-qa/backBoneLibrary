	...
	public void search_radius_selection() throws InterruptedException, IOException ,Exception
	{
		try {
			...
			for (String zeusUrl:OpenStreetMapLocatorPage.zeusUrl) {
				...
				test33.log(LogStatus.INFO, "Zeus Url :"+zeusUrl);
				...
				try {
					AssertJUnit.assertEquals(driver.findElement(By.xpath("//span[4]")).getText(), "Drive it - Distance: 0.39 miles");
				...
				try {
					AssertJUnit.assertEquals(driver.findElement(By.xpath("//span[4]")).getText(), "Drive it - Distance: 0.71 miles");
					test33.log(LogStatus.PASS, "Pass");
					System.out.println("Pass");
				} catch (AssertionError e) {
					System.out.println("FAIL :"+e.getMessage());
